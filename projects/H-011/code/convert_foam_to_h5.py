...
def find_datasets(raw_root: Path) -> List[FoamDataset]:
    """
    Identify foam datasets under raw_root (recursively).

    A dataset folder is any folder containing:
    - a "*BD.xlsx" file
    - at least one PNG frame (found recursively inside that folder)
    """
    datasets: List[FoamDataset] = []
    seen: set[Path] = set()
    for bd_file in raw_root.rglob("*BD.xlsx"):
        folder = bd_file.parent
        # Skip this repo folder if the user points raw_root above everything
        if "DTMicroscope-main" in folder.parts:
            continue
        # Avoid duplicates if multiple BD files exist in same folder
        if folder in seen:
            continue
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("."),
        help="Folder that contains or encloses the raw foam folders (recursively searched for *BD.xlsx)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("DTMicroscope-main/data/foam"),
        help="Destination folder for .h5 files",
    )
